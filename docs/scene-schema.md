...
2. **화면 영역 필터** — `category != 'display'`이고 item bbox 면적의
   **70% 이상**이 어떤 `screen_regions` 항목과 겹치면 그 item을 버린다.
   화면에 뜬 내용을 물건으로 오인한 것. `display`를 빼는 이유: 모니터
   자체의 bbox는 곧 자기 화면 bbox라 겹침이 항상 100%에 가깝다 — 실제
   실행(§16)에서 모니터가 두 사진 모두 이 필터에 잘못 걸려서 알았다.
...
## 16. 실측 결과 (2026-09-02, `tools/parse_scene.py` 실제 실행)

사용자가 준 `GEMINI_API_KEY`로 처음 실제 네트워크 호출을 했다. 사진은
C2(정면)·D(top_down) — §11의 그 사무실 책상 사진 두 장. `runs/2026-09-02/`에
원본 응답과 채점 결과가 있다.
...
다섯 지표 전부, 두 장 모두 합격선을 넉넉히 넘었다. `hit`/`crop` 채점은 이
세션이 두 사진을 다시 직접 열어보고 한 것 — desk1 테스트와 달리 이번엔
스크립트·스키마·프롬프트 전부 문서 그대로다.

**버그를 하나 실행 중에 잡았다.** 첫 실행에서 두 사진 모두 모니터
(`category: display`)가 "화면 영역"으로 잘못 버려졌다. 모니터 자체의 bbox가
곧 자기 화면 bbox라서(둘 다 거의 같은 사각형) §5-2의 겹침 필터(≥70%)에
걸린 것 — "화면 속 내용"과 "모니터라는 물건"을 구분 못 하고 있었다. 이건
스키마·프롬프트를 아무리 읽어도 안 보이고 실제로 돌려봐야 나오는 종류의
결함이었다. `postprocess()`에서 `category == 'display'`는 이 필터를
건너뛰도록 고쳤다 (§5에 반영).

**하나는 애매하게 남았다가 확인됐다.** C2의 `item_14`("소형 탁상시계")를
사진만으로는 특정 못 해 `n`(실재하나 이름틀림)으로 채점해뒀는데, 사용자
확인 결과 **뽀모도로 타이머**였다. 물건은 정확히 짚었고 용도를 잘못 읽은
경우 — 생김새(작고 둥근/각진 탁상용 물체)는 맞혔지만 기능을 오인했다.
`device` 카테고리 자체는 바뀌지 않는다. 라벨 정확도 93%는 그대로 두고
`runs/2026-09-02/C2-office-frontal-0902.items.csv`에 정답을 남겨뒀다.

## 17. 아직 없는 것

- **매칭 정확도.** 같은 책상을 두 번 찍은 쌍으로 재는 지표. C2·D는 같은
  순간·같은 책상을 두 각도로 찍은 것이라 "재촬영 매칭"의 대체재가 못 된다
  (§12에서 이미 다룬 문제 — 각도가 다르면 물건 집합이 달라 보인다). 진짜
  재촬영 쌍(C→C2, 5일 간격)으로 다시 재야 한다.
- **안정성.** 같은 사진 2회 호출 시 항목 수 편차. 아직 미측정.
- **비용 실측.** §8에서 미확인으로 남겨둔 것. `runs/2026-09-02/summary.txt`에
  토큰 수는 있으나(C2: in 2236 / out 3918, D: in 2236 / out 1500) Gemini
  요금으로 환산은 아직.
