    ...
    "items": {
      ...
      "items": {
        ...
        "required": [
          ...
          "zone", "sediment", "member_count", "splittable", "activity"
        ],
        "properties": {
          "id":       { "type": "string" },
  ...
  "$defs": {
    ...
    "category": {
      "enum": [
        ...
        "drinkware", "food_drink", "storage", "lighting", "plant", "decor",
        "device", "tool", "care", "textile", "trash", "unknown"
      ]
    }
  }
}
...
18개는 책상 한 판을 덮기에 충분하다고 보고 시작하되, 검증에서 `unknown`
...
| `writing` | 펜, 연필, 지우개, 자, 펜꽂이 |
| `paper` | 노트, 책, 서류, 포스트잇 |
| `drinkware` | 머그, 텀블러, 티팟, 잔, 코스터 |
| `food_drink` | 간식, 음료, 원두, 티백 |
| `storage` | 트레이, 케이스, 파우치, 서랍장, 정리함 |
| `lighting` | 스탠드, 조명, 무드등 |
| `plant` | 화분, 조화 |
...
3. **상판 필터** — bbox 중심이 `surface.bbox` 밖이면 버린다 (벽, 바닥, 창밖).
...
1. 정면 위에서 찍은 깔끔한 책상 (top_down, 기준선)
2. 비스듬히 찍은 평소 상태 (oblique — 실제 사용 각도)
3. 어수선한 상태 (군집·가림이 많은 최악 사례)
4. 2번과 같은 책상을 배치만 바꿔 다시 촬영 (매칭 검증용)
